...
previous=pd.read_csv(name)
for i in range(12):
    t = np.mean(previous.ix[:,1:i+2],axis=1)
    tt = np.array([t,t,t,t,t,t,t]).T
    a = np.argmin(np.abs(tt - label_range),axis=1) +1
    frame4.append(pd.DataFrame({'Month':np.ones(len(previous))*i + 201501,'IMSI':previous.IMSI, 'Label':a}))
...
train = X.ix[:,(new.columns != 'Month') & (X.columns != 'Brand') & (X.columns != 'Model') & (X.columns != 'IMSI') & (X.columns != 'Unnamed: 0')]
...
#second round
train = new.ix[:,(new.columns != 'Month') & (X.columns != 'Brand') &(new.columns != 'Model') & (new.columns != 'IMSI') & (new.columns != 'Unnamed: 0')]
...
t1 = np.vstack((prob[-1],prob[:-1]))
t2 = np.vstack((prob[-2],prob[-1],prob[:-2]))
new['L3P'] = pd.Series(t1[:,0], index=new.index)
new['L3N'] = pd.Series(t1[:,1], index=new.index)
new['L4P'] = pd.Series(t2[:,0], index=new.index)
new['L4N'] = pd.Series(t2[:,1], index=new.index)

T = (df.Month != 201503) & (df.Month != 201504) & (df.Month != 201510) & (df.Month != 201511) & (df.Month != 201512) & (df.Month != 201501) & (df.Month != 201502)
T = T.reset_index()
y=label.label[T.Month]
X = new[(new.Month != 201503) & (new.Month != 201504) & (new.Month != 201510) & (new.Month != 201511) & (new.Month != 201512)& (new.Month != 201501) & (new.Month != 201502)]
Unused_Feature = (X.columns != 'Brand') & (X.columns != 'Model') & (X.columns != 'IMSI') & (X.columns != 'Unnamed: 0')
train = X.ix[:,(new.columns != 'Month') & (X.columns != 'Brand') & (X.columns != 'Model') & (X.columns != 'IMSI') & (X.columns != 'Unnamed: 0')]
X_train, X_test, y_train, y_test=train_test_split(train, y, test_size = Test_Percentage, random_state = np.random.randint(100000))
